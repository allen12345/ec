 ...
 */
function cos_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'Ecommerce Pro';
...
function _wr_profile_install_configure_form_submit($form, &$form_state) {
  ...
  drupal_set_message("Ecommerce Pro Easy Installation was a Success! Imported $count queries.");
  drupal_set_message("For security purposes, it's recommended that you delete $sql_file, or move the Ecommerce Pro profile folder out of your webroot.");
...
function _wr_profile_prefixTables($sql) {
  ...
  foreach ($prefixes as $key => $val) {
    $sql = strtr($sql, array('prefixecommercepro_' . $key  => $val . $key));
  }
  // Then replace remaining tables with the default prefix.
  return strtr($sql, array('prefixecommercepro_' => $defaultPrefix ));
}
